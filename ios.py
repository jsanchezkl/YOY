...
DEST_BUCKET       = "app_yoy"
DATASET_ID        = "YOY"
APP_ID            = "id1618263486"         # Ajusta si iOS
...
TABLE_NAMES = {
    "t=installs":                "installs_ios",
    "t=inapps":                  "inapps_ios",
    "t=conversions_retargeting": "conversions_retargeting_ios",
    "t=inapps_retargeting":      "inapps_retargeting_ios",
}

def _get_dates_from_context(context):
    """
    Devuelve run_date (BogotÃƒÂ¡) y process_date (run_date-1, AAAA-MM-DD).
    """
    ...
    process_date  = (exec_dt_local).strftime("%Y-%m-%d")       # Sequita el - timedelta(days=1) porq airflow ya tiene un logicaltime de -1 dia 
...
def extract_and_prepare(**context):
    ...
    with tempfile.TemporaryDirectory(prefix="af_dl_") as workdir:
        ...
        for sub in TARGET_SUBFOLDERS:
            ...
            if not csv_files:
                log.warning("Sin CSV en %s para %s Ã¢â‚¬â€œ se omite", sub, process_date)
            ...
            gcs_path = f"processedios/{process_date}/{sub}/{sub}.csv"
            dst_bucket.blob(gcs_path).upload_from_filename(local_csv)
            log.info("Ã¢â€“Âº %s: %s filas Ã¢â€ â€™ gs://%s/%s", sub, row_count_sub, DEST_BUCKET, gcs_path)
            shutil.rmtree(local_dir, ignore_errors=True)

        log.info("Proceso AppsFlyer %s completado Ã¢â‚¬Â¢ Total filas: %s", process_date, total_rows)


# Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ ConversiÃƒÂ³n de tipos Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
...
# Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Load a BigQuery Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
def load_csv_to_bq(sub: str, **context):
    log = logging.getLogger("airflow.task")
    run_date, process_date = _get_dates_from_context(context)
    log.info("Cargando BigQuery Ã¢â‚¬Â¢ subcarpeta: %s | run_date: %s | process_date: %s",
    ...
    if not blob.exists():
        log.warning("No existe %s Ã¢â‚¬â€œ se salta carga", gcs_path)
...
# Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ DAG Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
default_args = {
    "owner": "data-eng",
    "retries": 1,
...
with DAG(
    dag_id="appsflyer_yoy_etl_ios",
    default_args=default_args,
    description="ETL AppsFlyer Ã¢â€ â€™ GCS Ã¢â€ â€™ BigQuery (tipos segÃƒÂºn esquema)",
    schedule_interval="0 1 * * *",                           # 01:00 AM BogotÃƒÂ¡
